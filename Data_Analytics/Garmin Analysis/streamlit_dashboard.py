...
def load_data():
    ...
    csv_file = garmin_folder / 'garmin_activities.csv'
    
    if not csv_file.exists():
        st.error(f"File {csv_file} not found! Click 'Download New Activities' to fetch data.")
        return None
    
    df = pd.read_csv(csv_file)
    
    # Rename columns to match dashboard expectations (English to Romanian)
    df = df.rename(columns={
        ...
        'Calories': 'Calorii',
        'Avg_HR': 'Ritm cardiac mediu',
        'Max_HR': 'Ritm cardiac maxim',
        'Elevation_Gain': 'Altitudine câștig',
        'Elevation_Loss': 'Altitudine pierdere'
    })
...
if df is not None:
    ...
    with col2:
        # Elevation Gain
        df_alt = df_filtered[df_filtered['Altitudine câștig'] > 0]
        ...
        if not df_alt.empty:
            ...
            fig_alt.add_trace(go.Bar(
                x=df_alt['Data'],
                y=df_alt['Altitudine câștig'],
                marker_color='rgb(148, 103, 189)',
                hovertemplate='<b>%{x}</b><br>Elevation Gain: %{y} m<extra></extra>'
            ))
